...
namespace Pgyf\Opensdk\Kernel\Socialite\Providers;

// use GuzzleHttp\Client as GuzzleClient;
// use GuzzleHttp\Utils;
use Psr\Http\Message\MessageInterface;
use Psr\Http\Message\StreamInterface;
use Pgyf\Opensdk\Kernel\Socialite\Config;
use Pgyf\Opensdk\Kernel\Socialite\Contracts;
use Pgyf\Opensdk\Kernel\Socialite\Exceptions;
use Pgyf\Opensdk\Kernel\Socialite\GuzzleClient;
use Pgyf\Opensdk\Kernel\Support\Json;
use Pgyf\Opensdk\Kernel\Socialite\GuzzleResponse;
...
abstract class Base implements Contracts\ProviderInterface
{
     ...
     * @var string
     */
    protected       $expiresInKey = Contracts\RFC6749_ABNF_EXPIRES_IN;
    /**
     * @var string
     */
    protected       $accessTokenKey = Contracts\RFC6749_ABNF_ACCESS_TOKEN;
    /**
     * @var string
     */
    protected       $refreshTokenKey = Contracts\RFC6749_ABNF_REFRESH_TOKEN;
    ...
    protected function normalizeAccessTokenResponse($response): array
    {
        ...
        if (empty($response[$this->accessTokenKey])) {
            throw new Exceptions\AuthorizeFailedException('Authorize Failed: ' . Json::encode($response, \JSON_UNESCAPED_UNICODE), $response);
        }
    ...
    protected function fromJsonBody(GuzzleResponse $response): array
    {
